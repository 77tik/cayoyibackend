...
const SingleModelInfo: React.FC<SingleModelInfoProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      if (queryParams) {
        try {
          setLoading(true);
          const result = (await getFluidResult(queryParams)) as {
  ...
  }, [activeCard, activeButton, profileType, simulationResult, queryParams]);
  ...
  return (
    <SingleModelWrapper>
      ...
      {/* VirtualProbe 探针，仅在容器和资源管理器都存在时渲染 */}
