...
// 定义API响应类型
interface StructuralConditionResponse {
  data: {
    data: {
...
const MultiConditionPanel: React.FC<MultiConditionPanelProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response =
          (await getStructuralConditions()) as StructuralConditionResponse;
        const structuralData = response.data.data.conditions;
        setConditions(structuralData);
        if (structuralData.length > 0 && !selectedHead && !selectedPower) {
          // 只在状态为空时设置默认值
          const firstHead = Array.from(
            new Set(structuralData.map((c) => c.effective_head)),
          ).sort((a, b) => a - b)[0];
          const firstPower = Array.from(
            new Set(structuralData.map((c) => c.active_power)),
          ).sort((a, b) => b - a)[0];
          onMultiConditionStateChange({
