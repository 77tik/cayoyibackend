...
func newClusterNodeGroups() *ClusterNodeGroups {
	return &ClusterNodeGroups{
		groupMembers: make(map[FilerGroupName]*GroupMembers),
	}
}
...
func (g *ClusterNodeGroups) ListClusterNode(fileGroup FilerGroupName) (nodes []*ClusterNode) {
	g.Lock()
	defer g.Unlock()
	m := g.getGroupMembers(fileGroup, false)
	if m == nil {
		return nil
	}
