...
// 远端存储模块，通过集成 rclone 支持将数据读写到对象存储服务（如 S3、GCS、阿里云 OSS、OneDrive、Dropbox 等）。
// 它的核心作用是让 SeaweedFS 的 Volume 数据可以通过 Rclone 存储到任意兼容的远程云存储系统中。
...
type RcloneBackendStorage struct {
	id              string
	remoteName      string             // 远程 rclone 存储名（例如 s3、gdrive 等
	keyTemplate     *template.Template // 键模板用于生成远程文件路径
	keyTemplateText string
	fs              fs.Fs // Rclone 文件系统接口
}
...
// 远程文件抽象（单个文件），为了对内使用，实现backend存储接口
