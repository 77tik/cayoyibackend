...
// Erasure Coding（纠删码），用来将一个 .dat 文件编码成多个数据分片（.ec00 ~ .ec09），可以在部分丢失的情况下恢复数据。
// 在编码之后，原始 .dat 和 .idx 会被删除，剩下多个 .ec** 文件和两个索引文件：
// .ecx: 保存存活文件的元数据（key, offset, size）
// .ecj: 保存删除标记的 key 列表
// | 文件名后缀              | 说明                                |
// | ------------------ | --------------------------------- |
// | `.ec00` \~ `.ec05` | 数据分片（6个）                          |
