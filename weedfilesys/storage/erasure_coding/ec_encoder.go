...
// | ------------- | ----------------------------- |
// | `.dat`        | 完整的数据文件                       |
// | `.idx`        | 索引文件，记录 `.dat` 中每个 Needle 的位置 |
// | `.ec00~.ec09` | 数据分片（Data Shards）             |
// | `.ec10~.ec13` | 冗余分片（Parity Shards）           |
// | `.ecx`        | `.idx` 中数据部分（已排序）             |
// | `.ecj`        | 删除的 NeedleId                  |
...
//某个 .dat 文件完成写入
//
//调用 WriteEcFiles() → 生成 .ec00~.ec13
//
//同时生成 .ecx → 用于未来恢复 .idx
//
//如果 .ec02 丢失，调用 RebuildEcFiles() 自动恢复它
//
//.ecx 和 .ecj 可用于未来恢复 .idx 文件
