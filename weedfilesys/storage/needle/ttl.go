...
// 文件的生存时间模块,支持 3d 转换为时间
type TTL struct {
	Count byte // 时间数量，例如：3d 中的 3
	Unit  byte // 时间单位，例如：3d 中的 'd'（Day）
}
...
func ReadTTL(ttlString string) (*TTL, error) {
	...
	unitBytes := ttlBytes[len(ttlBytes)-1]
	countBytes := ttlBytes[:len(ttlBytes)-1]
	if '0' <= unitBytes && unitBytes <= '9' {
		countBytes = ttlBytes
		unitBytes = 'm'
	}
	count, err := strconv.Atoi(string(countBytes))
	unit := toStoredByte(unitBytes)
