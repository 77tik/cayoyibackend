...
// 内存索引结构：CompactMap，用于高效地在内存中存储和查找每个文件（Needle）的位置索引（即 .idx 文件内容的内存映射）。
// 每个 Volume 都需要一个快速的内存结构来查找某个文件数据的位置：
// 当你要读取一个文件 → 先查 NeedleMap 找 offset → 去 .dat 文件中定位并读取。
// 当你要写入一个文件 → 更新 NeedleMap，记录新 offset 和 size。
// 当你要删除一个文件 → 更新 NeedleMap 中 size 为负数表示删除。

// Needle 是真正的 数据行（row）
// NeedleMap 是对应的 索引（index）
// .dat 文件存储的是 Needle 的数据
// .idx 文件（加载到 NeedleMap）存的是每条 Needle 的 (key, offset, size)
